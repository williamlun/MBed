...
{
    if(!RPDO1_EXE(node_num, 0x010F)){PC_SEND("    MOTION ERROR: FAIL TO STOP PROFILE VELOCITY\r\n"); return 0;}
    //PC_SEND("STOP PROFILE VELOCITY\r\n");
    return 1;
}
...
{
    if(!RPDO1_EXE(node_num, 0x000B)){PC_SEND("    MOTION ERROR: FAIL TO QUICK STOP PROFILE VELOCITY\r\n"); return 0;}
    //PC_SEND("QUICK STOP PROFILE VELOCITY\r\n");
    return 1;
}
...
{
    if(!RPDO1_EXE(node_num, 0x0002)){PC_SEND("    MOTION ERROR: FAIL TO QUICK STOP CURRENT MODE VELOCITY\r\n"); return 0;}
    //PC_SEND("QUICK STOP CURRENT MODE VELOCITY\r\n");
    return 1;
}
...
{
    if(!SDO_EXE(node_num,0x65,0x60,0x00,data,4)){PC_SEND("    MOTION ERROR: FAIL TO SET MAXIMUM FOLLOWING ERROR\r\n"); return 0;}
...
{
    if(!SDO_EXE(node_num,0x7D,0x60,0x01,data,4)){PC_SEND("    MOTION ERROR: FAIL TO SET MINIMUM POSITION LIMIT\r\n"); return 0;}
...
{
    if(!SDO_EXE(node_num,0x7D,0x60,0x02,data,4)){PC_SEND("    MOTION ERROR: FAIL TO SET MAXIMUM POSITION LIMIT\r\n"); return 0;}
...
{
    if(!SDO_EXE(node_num,0x81,0x60,0x00,data,4)){PC_SEND("    MOTION ERROR: FAIL TO SET PROFILE VELOCITY\r\n"); return 0;}
...
{
    if(!SDO_EXE(node_num,0x7A,0x60,0x00,data,4)){PC_SEND("    MOTION ERROR: FAIL TO SET TARGET POSITION\r\n"); return 0;}
...
{
    if(!SDO_EXE(node_num,0x40,0x60,0x00,0x001F,2)){PC_SEND("    MOTION ERROR: FAIL TO START ABSOLUTE POSITION\r\n"); return 0;}
...
{
    if(!RPDO1_EXE(node_num, 0x001F)){PC_SEND("    MOTION ERROR: FAIL TO START ABSOLUTE POSITION\r\n"); return 0;}
    //PC_SEND("START ABSOLUTE POSITION\r\n");
    return 1;
}
...
{
    if(!SDO_EXE(node_num,0x40,0x60,0x00,0x003F,2)){PC_SEND("    MOTION ERROR: FAIL TO START ABSOLUTE IMMIDIATE POSITION\r\n"); return 0;}
...
{
    if(!RPDO1_EXE(node_num, 0x003F)){PC_SEND("    MOTION ERROR: FAIL TO START ABSOLUTE IMMIDIATE POSITION\r\n"); return 0;}
    //PC_SEND("START ABSOLUTE IMMIDIATE POSITION\r\n");
    return 1;
}
...
{
    if(!SDO_EXE(node_num,0x40,0x60,0x00,0x005F,2)){PC_SEND("    MOTION ERROR: FAIL TO START RELATIVE POSITION\r\n"); return 0;}
...
{
    if(!RPDO1_EXE(node_num, 0x005F)){PC_SEND("    MOTION ERROR: FAIL TO START RELATIVE POSITION\r\n"); return 0;}
    //PC_SEND("START RELATIVE POSITION\r\n");
    return 1;
}
...
{
    if(!SDO_EXE(node_num,0x40,0x60,0x00,0x007F,2)){PC_SEND("    MOTION ERROR: FAIL TO START RELATIVE IMMEDIATE POSITION\r\n"); return 0;}
...
{
    if(!RPDO1_EXE(node_num, 0x007F)){PC_SEND("    MOTION ERROR: FAIL TO START RELATIVE IMMEDIATE POSITION\r\n"); return 0;}
    //PC_SEND("START RELATIVE IMMEDIATE POSITION\r\n");
    return 1;
}
...
{
    if(!SDO_EXE(node_num,0x40,0x60,0x00,0x010F,2)){PC_SEND("    MOTION ERROR: FAIL TO STOP POSITIONING\r\n"); return 0;}
...
{
    if(!RPDO1_EXE(node_num, 0x010F)){PC_SEND("    MOTION ERROR: FAIL TO STOP POSITIONING\r\n"); return 0;}
    //PC_SEND("STOP POSITIONING\r\n");
    return 1;
}
...
{
    if(!SDO_EXE(node_num,0x40,0x60,0x00,0x000B,2)){PC_SEND("    MOTION ERROR: FAIL TO QUICK STOP POSITIONING\r\n"); return 0;}
...
{
    if(!RPDO1_EXE(node_num, 0x000B)){PC_SEND("    MOTION ERROR: FAIL TO QUICK STOP POSITIONING\r\n"); return 0;}
    //PC_SEND("QUICK STOP POSITIONING\r\n");
    return 1;
}
...
bool SET_HOMING_MODE(uint8_t node_num, HOMING_MODE mode)
{
    ...
    if(!SDO_EXE(node_num,0x98,0x60,0x00,hmode,1)){PC_SEND("    MOTION ERROR: FAIL TO SET HOMING MODE\r\n"); return 0;}
...
{
    if(!SDO_EXE(node_num,0x7C,0x60,0x00,data,4)){PC_SEND("    MOTION ERROR: FAIL TO SET HOME OFFSET\r\n"); return 0;}
...
{
    if(!SDO_EXE(node_num,0x99,0x60,0x02,data,4)){PC_SEND("    MOTION ERROR: FAIL TO SET SPEED FOR ZERO SEARCH\r\n"); return 0;}
...
{
    if(!SDO_EXE(node_num,0x9A,0x60,0x00,data,4)){PC_SEND("    MOTION ERROR: FAIL TO SET HOMING ACCELEARATION\r\n"); return 0;}
...
{
    if(!SDO_EXE(node_num,0x80,0x20,0x00,data,2)){PC_SEND("    MOTION ERROR: FAIL TO SET HOMING CURRENT THRESHOLD\r\n"); return 0;}
...
{
    if(!SDO_EXE(node_num,0x81,0x20,0x00,data,4)){PC_SEND("    MOTION ERROR: FAIL TO SET HOME POSITION\r\n"); return 0;}
...
{
    if(!SDO_EXE(node_num,0x40,0x60,0x00,0x001F,2)){PC_SEND("    MOTION ERROR: FAIL TO START HOMING\r\n"); return 0;}
...
{
    if(!RPDO1_EXE(node_num, 0x001F)){PC_SEND("    MOTION ERROR: FAIL TO START HOMING\r\n"); return 0;}
    //PC_SEND("START HOMING\r\n");
    return 1;
}
...
{
    if(!SDO_EXE(node_num,0x40,0x60,0x00,0x011F,2)){PC_SEND("    MOTION ERROR: FAIL TO STOP HOMING\r\n"); return 0;}
    //PC_SEND("STOP HOMING\r\n");
    return 1; 
}
...
{
    if(!RPDO1_EXE(node_num, 0x011F)){PC_SEND("    MOTION ERROR: FAIL TO STOP HOMING\r\n"); return 0;}
    //PC_SEND("STOP HOMING\r\n");
    return 1;
}
...
{
    if(!SDO_EXE(node_num,0x40,0x60,0x00,0x000B,2)){PC_SEND("    MOTION ERROR: FAIL TO QUICK STOP HOMING\r\n"); return 0;}
...
{
    if(!RPDO1_EXE(node_num, 0x000B)){PC_SEND("    MOTION ERROR: FAIL TO QUICK STOP HOMING\r\n"); return 0;}
    //PC_SEND("QUICK STOP HOMING\r\n");
    return 1;
}

...
float wheelR = 0.1524/2; //4 inch wheel  //meter
float gear = 12.5;
...
struct point_info  //message
{
   float required_x;
   float required_y;
   float required_w; //in degree
   float required_tolerance_x;
   float required_tolerance_y;
   float required_tolerance_w;  //in rad
   float required_speed_max_x;
   float required_speed_max_y;
   float required_speed_max_w;  //rad ps
   float pid_on;    // 0 or 1
};
...
void calculatePos(float _X,float _Y,float _A)       //pull the 
{
...
int main() {    //point[2] is the first point
